...
function setup() {
    ...
    weatherField = createInput("Sunny");
    weatherField.position(windowWidth / 2 - 600, 50);
    weatherField.size(200, 20);

    locationField = createInput("New York City");
    locationField.position(windowWidth / 2 - 600, 80);
    locationField.size(200, 20);

    emotionField = createInput("Happy");
    emotionField.position(windowWidth / 2 - 600, 110);
    emotionField.size(200, 20);

    actionField = createInput("Sleeping");
    actionField.position(windowWidth / 2 - 600, 140);
    actionField.size(200, 20);

    let madlibButton = createButton("Create Madlib");
    madlibButton.position(windowWidth / 2 - 600, 170);
    madlibButton.mousePressed(function () {
        madLibStory = "It is a " + weatherField.value() + " day in " + locationField.value()  + ". As I am " + actionField.value()  + ", I am in a " + emotionField.value()  + " mood."
        inputField.value(madLibStory);
        imageInputField.value(madLibStory);
        // fill(255);
    });

    imageInputField = createInput("");
    imageInputField.position(windowWidth / 2 - 350, 50);
    imageInputField.size(200, 20);

    let askImageButton = createButton("Ask For Image");
    askImageButton.position(windowWidth / 2 - 350, 80);
    askImageButton.mousePressed(function () {
        console.log('image gen pressed')
        askForPicture(imageInputField)
    });
